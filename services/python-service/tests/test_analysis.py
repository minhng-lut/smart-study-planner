...
class AnalysisServiceTests(unittest.TestCase):
    def test_completed_tasks_have_none_risk_and_do_not_affect_workload(self):
        ...
        self.assertEqual(result.task_priorities[0].priority_score, 0)

    def test_overdue_task_is_high_risk_and_ranked_first(self):
        result = analyze(
            AnalyzeRequest(
                current_datetime=datetime(2026, 4, 19, 9, 0, tzinfo=UTC),
                tasks=[
        ...
        self.assertEqual(result.task_risk_levels[0].risk_level, "high")
        self.assertEqual(result.task_priorities[0].task_id, 10)
        self.assertGreater(
